...
- **D. Дистрибуция + доки:** Hangar (PaperMC) / Modrinth / SpigotMC; мини-сайт доков (API reference, примеры, migration), OG-обложка, copy-to-clipboard в коде.
  ...
  - ⏳ **Hangar — частично.** Проект создан: **https://hangar.papermc.io/DARKPIX/PaperScript** (id 6529, slug `PaperScript`, category dev_tools, owner DARKPIX, links GitHub/Docs/Issues, лицензия MIT, pageContent из README). **Версии НЕ загружены:** присланный API-ключ имеет permissions `111` (только биты 0–2: view_public_info, edit_own_user_settings, edit_api_keys) → `POST /upload` отвечает голым 404 (PermissionRequired CREATE_VERSION скрывает проект). Эскалация через `POST /api/v1/keys` невозможна (`apiKeys.error.notEnoughPerms` — ключ может создавать только ключи-подмножества). **Нужен новый ключ с `create_version` (+желательно edit_version, edit_page, edit_subject_settings)** — создать в веб-UI Hangar (профиль → API keys). Залить: modern `0.2.0` → PAPER `1.18.x/1.19.x/1.20.x/1.21.x` (jar 36MB — nginx даёт 413 на прямой upload → использовать `files[0].externalUrl` на GitHub release asset), legacy `0.2.0-legacy` → PAPER `1.12.2/1.16.5` (42KB, можно файлом). Готовые payload'ы: `/c/Programming/.tmp-target/vu_modern.json` / `vu_legacy.json` (+jar'ы там же). JWT: `POST /api/v1/authenticate?apiKey=KEY` → `Authorization: HangarAuth <jwt>` (TTL ~короткий, перевыпускать).
