...
Текущий выбор пользователя: **A (production-trust) + C (security/DX)**. A1/A2/A3/C — **сделано**: CI зелёный (`b50fb43`, real-Paper e2e PASS), `@paperscript/sdk@0.1.0` опубликован в npm (org `paperscript`, install+CLI проверены). Побочно: `@darkpix/supercell-api@3.1.0` тоже опубликован (ребренд с vladnet14). **Точка возобновления: B (legacy 1.12.2/1.16.5) и D (дистрибуция/доки).**
