...
- **D. Дистрибуция + доки:** Hangar (PaperMC) / Modrinth / SpigotMC; мини-сайт доков (API reference, примеры, migration), OG-обложка, copy-to-clipboard в коде.
  ...
  - ⏳ **Hangar — ждёт API-ключ от пользователя.** Когда будет: Hangar API v1, проект PaperScript (Developer Tools), версии: modern jar → PAPER 1.18–1.21.x, legacy jar → PAPER 1.12.2/1.16.5.
