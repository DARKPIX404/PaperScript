...
public final class ScriptPlayer {
    ...
    /** Sends a message with legacy '&' color codes (no MiniMessage on 1.12.2). */
    public void sendMessage(String message) {
        handle.sendMessage(LegacyChat.render(message));
    }
    ...
    // Package-private on purpose: raw Bukkit access stays host-only (guest code
    // only sees PUBLIC members of bound facade objects).
    Player handle() {
        return handle;
    }
}
